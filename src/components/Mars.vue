<template>
  <div> 
    ...
    <div class="row" v-if="hasOpenLoading != true" id = "bodymars">
    <!-- 12. เนื่องจากเราทำการอัพเดตรค่าของ hasOpenLoading = false ในบรรทัดที่ 47 จึงสามารถทำงาน ชุดคำสั่งตั้งแต่ บรรทัดที่ 11 ถึง 14 ได้ -->
       <div class="centerx icons-example row">
       <vs-col  id="">
       {{inputsearch}}
       <vs-input icon="search"  placeholder="Search" v-model="inputsearch"/>
      </vs-col>
      </div>
      <div class = "row">
      <div class="col " v-for="item in marssearch.photos" v-bind:key="item.index">
...
export default {
  ...
  // 3. async function ทำการเรียกใช้ axios เพิ่อดึงข้อมูลจาก api
     ...
     search: function () {
       ...
       this.marssearch = this.mars.photos.filter(s => s.camera.full_name = this.inputsearch )
     },
  },
  // 1.
  created() {
